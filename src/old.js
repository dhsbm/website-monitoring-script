...
export default class Tracker {
  ...
  constructor(options) {
    if (!options.requestUrl) options.requestUrl = 'http://localhost:3000/report/error'
          ...
          window.onerror = (message, source, lineno, colno, error) => {
            ...
            if (error != null) {
              ...
              this.#reportTracker({
                ...
                line: '' + (lineno || ''), //错误所在行号
                col: '' + (colno || ''), //错误所在列号
                stack: error.stack ? error.stack.split('at').map((item) => item.trim()) : '', // 错误的堆栈信息
        ...
        function reportAssets() {
          ...
          if (resourceEntries.length) {
            resourceEntries.forEach((entry) => {
              let status = resourceErrorList.includes(entry.name) ? 'Failed' : 'Success'
              self.#reportTracker({
